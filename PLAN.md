...
Initial target: 1800 bytes of compressed binary per chunk.
...
If local testing proves reliable at larger values, it can be raised toward 2800. Reliability matters more than theoretical transfer speed.
...
Correction: start at 1800 compressed bytes per chunk, keep it as a constant, and treat 2800 as an optimization only after scan reliability is proven.
...
At 1800-byte chunks, a compressed 100 MB file could require tens of thousands of QR codes and hours of optical transfer. Browser memory pressure will also be high if every JSON payload and QR string is precomputed.
...
- Start with `CHUNK_SIZE_BYTES = 1800`.
